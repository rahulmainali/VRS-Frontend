...
function Rentals() {
  ...
  const fetchData = async (userId: number) => {
    const response: any = await axios(`${url}/getRentals/${userId}`).catch(
      err => console.log(err)
    )

    setData(response.data.rentals)
    ...
    <div className=" w-full p-0 px-5 float-right h-screen ">
      ...
      >
        <div className="mx-auto w-full px-5">
          <h1 className="text-left text-2xl font-semibold py-3">My Rentals</h1>
